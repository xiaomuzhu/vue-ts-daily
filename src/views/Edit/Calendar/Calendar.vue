...
@Component({
  ...
  export default class Calendar extends Vue {
    ...
    @Mutation private saveLog: (payload: {
      id: number,
      daysId: number,
      message: string,
    }) => void;
    @Mutation private changeFinished: (payload: {
      id: number,
      daysId: number,
    }) => void;
    @Mutation private supplementHabits: (payload: {
      id: number,
      daysId: number,
    }) => void;
    private data() {
    ...
    private get habitLogComputed() {
      ...
      const { totalHabitDays, mostConsecutiveDays, currentConsecutiveDays } = this.currentHabit!.habitLog;
      const createdTime = this.currentHabit!.id;
      // 创建至今多少天
      const totalTodays = _.getDaysId() - _.getDaysId(createdTime) + 1;
      // 创建日期
      moment.locale('zh-cn');
      const create = moment(createdTime).format('l').replace(/\//g, '-');
      return {
        totalHabitDays, mostConsecutiveDays, currentConsecutiveDays, totalTodays, create,
      }
    }
    private get habitEventsComputed() {
      ...
      const habitEvents = date.filter((item) => item.isFinished).map((ele) => {
        moment.locale('zh-cn');
        const date = moment(ele.time!).format('L').replace(/-/g, '/');
        return {
          date,
          title: ele.message,
        }
